...
class LstmParam:
    def __init__(self, layer_size, prepared_params=None, load=False):
        # weight matrices
...
class LstmState:
    def __init__(self, n_timesteps, n_samples, layer_size, param):
        ...
        self.l1_s = np.empty((n_timesteps, n_samples, layer_size[1]))
        self.l1_h = np.empty((n_timesteps, n_samples, layer_size[1]))
        ...
        self.l2_o = np.empty((n_timesteps, n_samples, layer_size[2]))
    ...
    def forward(self, t, xt):
        ...
        self.Y_hat[t] = log_softmax(self.l2_h[t])
...
class Lstm:
    ...
    def backward(self, batch_size, t, y_label, trunc_h= None, trunc_s= None):
        ...
        # dh[1] = (self.state.Y_hat[t] - y_label)/(softmax(self.state.l2_h[t])) #*sigmoid_derivative(self.state.l2_h[t]) 
        dh[1] = derivative_batched_l2_h(self.state.Y_hat[t], y_label, self.state.l2_h[t]) 
        # dh[1] = cross_entropy_derivatives(self.state.Y_hat[t], y_label)*derivative_log_softmax(self.state.l2_h[t])
